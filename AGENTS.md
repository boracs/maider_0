...
### Non-obvious gotchas
- **Case-sensitive import bug (breaks on Linux/CI):** several files import `@/Components/*` and `@/Layouts/*` (capital), but the real directories are `resources/js/components` and `resources/js/layouts` (lowercase). This works on case-insensitive macOS/Windows but fails on Linux with Vite `Failed to resolve import "@/Components/..."`, so the login/register/profile pages break. The startup update script creates compatibility symlinks (`resources/js/Components -> components`, `resources/js/Layouts -> layouts`) and git-excludes them so the app runs unmodified on Linux. **Do not commit these symlinks** (they collide with the real dirs on case-insensitive filesystems). The real fix is for the maintainer to correct the import casing to lowercase.
